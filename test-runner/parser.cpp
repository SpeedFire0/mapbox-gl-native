...
ArgumentsTuple parseArguments(int argc, char** argv) {
    ...
    auto ids = args::get(testNameValues);
    std::transform(ids.begin(), ids.end(), ids.end(), [testDefaultPath](const auto& id) { return testDefaultPath + id; });
...
TestMetadata parseTestMetadata(const mbgl::filesystem::path& path) {
    ...
    if (!maybeJson.is<mbgl::JSDocument>()) {
...
std::string createResultItem(const TestMetadata& metadata, bool hasFailedTests) {
    ...
    html.append("<h2><span class=\"label\" style=\"background: " + metadata.color + "\">" + metadata.status + "</span> " + metadata.id + "</h2>\n");
